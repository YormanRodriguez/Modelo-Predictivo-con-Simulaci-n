...
class ClimateModel(QObject):
    ...
    REGIONALES: ClassVar[dict[str, str]] = {
        "SAIDI_C": "Cúcuta",
        "SAIDI_O": "Ocaña",
        "SAIDI_A": "Aguachica",
        "SAIDI_P": "Pamplona",
    ...
    CLIMATE_COLUMNS: ClassVar[dict[str, str]] = {
    ...
    def load_climate_file(self, regional_code: str, file_path: str) -> bool:
        ...
        try:
            ...
            # Leer archivo
            df = self._read_climate_file(file_path)
            if df is None:
                return False

            # Validar y procesar
            if not self._validate_and_process_data(df, regional_code, regional_nombre, file_path):
                return False

            print(f"[DEBUG CLIMATE] {regional_nombre} cargada: {len(self._climate_data[regional_code])} meses válidos")

        except (KeyError, AttributeError, TypeError, OSError) as e:
            error_msg = f"Error inesperado cargando clima de {regional_code}: {e!s}"
    ...
    def _validate_initial_conditions(self, regional_code: str, file_path: str) -> bool:
        """Validar condiciones iniciales antes de cargar el archivo."""
    ...
    def _read_climate_file(self, file_path: str) -> pd.DataFrame | None:
        """Leer archivo climático (CSV o Excel)."""
        path = Path(file_path)
        ...
        try:
            if file_ext in [".xlsx", ".xls"]:
            ...
            elif file_ext == ".csv": # Usamos 'elif' para asegurar que solo se intente leer un tipo
                ...
                # Si el formato no es soportado, emitimos el error
                error_msg = f"Formato de archivo no soportado: {file_ext}. Use .xlsx, .xls o .csv"
                self.error_occurred.emit(error_msg)
                return None # Retorna None inmediatamente si no es soportado

        except (OSError, pd.errors.ParserError, pd.errors.EmptyDataError) as e:
            # Manejo de errores de lectura (incluyendo archivo no encontrado, parseo, o vacío)
            error_msg = f"Error leyendo archivo: {e!s}"
            self.error_occurred.emit(error_msg)
            return None

        else:
            # Este bloque 'else' se ejecuta SÓLO si el bloque 'try' finaliza
            # SIN levantar una excepción y si 'df' fue asignado (es decir,
            # si se leyó un archivo soportado).
            if df is not None:
                print(f"[DEBUG CLIMATE] Archivo leído: {len(df)} registros mensuales")
                return df
            # Si 'df' es None aquí, significa que se ejecutó el 'else' del formato no soportado
            # en el bloque 'try', y ya se retornó None. Esto es una doble verificación.
            return None

    def _validate_and_process_data(
        self, df: pd.DataFrame, regional_code: str, regional_nombre: str, file_path: str,
    ) -> bool:
        """Validar estructura y procesar datos climáticos."""
    ...
    def _validate_climate_structure(self, df: pd.DataFrame, regional_nombre: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Verificar columnas climáticas importantes
            required_cols = [
            ...
            # Validar formato de fechas y cantidad de datos (COMBINADAS)
            dates_parsed = pd.to_datetime(df[date_col], format="%Y-%m", errors="coerce")
            datos_no_acaptables = 6

            if dates_parsed.isna().all() or len(df) < datos_no_acaptables:
                if dates_parsed.isna().all():
                    error_msg = f'Columna "{date_col}" en {regional_nombre} no tiene formato válido (esperado: YYYY-MM)'
                else:
                    error_msg = f"{regional_nombre} tiene muy pocos datos mensuales ({len(df)}). Se requieren al menos 6 meses."
                return {"valid": False, "error": error_msg}

        except (KeyError, AttributeError, TypeError, ValueError) as e:
            return {"valid": False, "error": f"Error validando estructura: {e!s}"}
    ...
    def _process_climate_data(self, df: pd.DataFrame, regional_code: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            columns_to_extract = self._extract_available_columns(df_clean)

            # Crear DataFrame final
            df_final = self._build_final_dataframe(df_clean, columns_to_extract, regional_code)
            ...
            self._print_extraction_statistics(df_final, columns_to_extract)
            return df_final

    def _extract_available_columns(self, df: pd.DataFrame) -> dict[str, str]:
        """Extraer columnas disponibles del DataFrame."""
        excluded_keys = ["date", "year", "month", "valid_days", "total_records"]

        return {
            var_column: var_key
            for var_key, var_column in self.CLIMATE_COLUMNS.items()
            if var_key not in excluded_keys and var_column in df.columns
        }

    def _build_final_dataframe(
        self, df_clean: pd.DataFrame, columns_to_extract: dict[str, str], regional_code: str,
    ) -> pd.DataFrame:
        """Construir DataFrame final con todas las variables."""
    ...
    def _print_extraction_statistics(self, df_final: pd.DataFrame, columns_to_extract: dict[str, str]) -> None:
        """Imprimir estadísticas de extracción de variables."""
        excluded_keys = ["date", "year", "month", "valid_days", "total_records"]
        ...
        self._check_critical_variables(df_final)

        # Reporte de categorías
        self._print_variable_categories(df_final)

    def _check_critical_variables(self, df: pd.DataFrame) -> None:
    ...
    def _print_variable_categories(self, df: pd.DataFrame) -> None:
        """Imprimir variables agrupadas por categoría."""
        var_categories = {
            "Temperatura": [col for col in df.columns if "temp" in col or "heat" in col or "windchill" in col or "realfeel" in col],
            "Humedad": [col for col in df.columns if "humidity" in col or "dewpoint" in col],
            "Precipitación": [col for col in df.columns if "precip" in col or "rain" in col],
            "Presión": [col for col in df.columns if "pressure" in col],
            "Viento": [col for col in df.columns if "wind" in col],
            "Radiación": [col for col in df.columns if "solar" in col or "uv" in col or "illuminance" in col],
            "Evapotranspiración": [col for col in df.columns if "eto" in col],
            "Días_medición": [col for col in df.columns if "days" in col],
        }
    ...
    def _generate_climate_info(self, df: pd.DataFrame, regional_code: str, file_path: str) -> dict[str, Any]:
        ...
        info = {
            ...
            "file_name": Path(file_path).name,  # ✓ Path.name en lugar de os.path.basename
            "file_path": file_path,
    ...
    def get_climate_data(self, regional_code: str) -> pd.DataFrame | None:
    ...
    def get_missing_regionales(self) -> list[str]:
        ...
        return [code for code in self.REGIONALES if code not in self._climate_data]

    def get_climate_info(self, regional_code: str) -> dict[str, Any] | None:
    ...
    def export_summary_report(self) -> str:
        ...
        for regional_code in self.REGIONALES:  # ✓ Elimina .keys()
    ...
    def validate_analyzer_compatibility(self, regional_code: str) -> dict[str, Any]:
        ...
        result = {
            ...
            "present_vars": sorted(present_vars),      # ✓ Elimina list()
            "missing_vars": sorted(missing_vars),      # ✓ Elimina list()
            "extra_vars": sorted(extra_vars),          # ✓ Elimina list()
    ...
    def print_compatibility_report(self):
        ...
        for regional_code in self.get_loaded_regionales():
            ...
            compatibilidad_optima = 90
            compatibilidad_aceptable = 75
            compatibilidad_de_cuidado = 60

            if validation["compatibility_score"] >= compatibilidad_optima:
                status = " EXCELENTE"
            elif validation["compatibility_score"] >= compatibilidad_aceptable:
                status = " BUENA"
            elif validation["compatibility_score"] >= compatibilidad_de_cuidado:
                status = " ACEPTABLE"
            else:
                status = " INSUFICIENTE"
            ...
            completitud_excelente = 80
            completitud_mala = 50

            print("\nCompletitud por categoría:")
            for category, stats in validation["category_completeness"].items():
                completeness = stats["completeness_pct"]
                indicator = "Bueno" if completeness >= completitud_excelente else "Precacución" if completeness >= completitud_mala else "Peligro"
            ...
            if validation["missing_vars"]:
                ...
                for var in validation["missing_vars"][:comparasion]:
                    print(f"  - {var}")
                if len(validation["missing_vars"]) > comparasion:
                    print(f"  ... y {len(validation['missing_vars']) - comparasion} más")
    ...
    def get_compatibility_summary(self) -> dict[str, Any]:
        ...
        for regional_code in self.get_loaded_regionales():
            ...
            # Clasificar
            if score >= compatibilidad_optima:
                summary["overall_stats"]["excellent"] += 1
            elif score >= compatibilidad_aceptable:
                summary["overall_stats"]["good"] += 1
            elif score >= compatibilidad_de_cuidado:
